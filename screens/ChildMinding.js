...
import { ScrollView, View, Image, Text, StyleSheet, TouchableOpacity, params } from 'react-native';

const ChildMinding = ({ navigation }) => {
  const courseName = 'Child Minding';
  const coursePurpose = 'To provide basic child and baby care';
  const content1 = 'Birth to 6-month-old baby needs';
  const content2 = 'Seven months to one year old needs';
  const content3 = 'Toddler needs';
  const content4 = 'Educational Toys';
  const price = 750;

  return (
    <ScrollView style={styles.container}>
      <Image source={require('../Assets/ChildMinding.jpg')} style={styles.headerImage} />
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate('Info', {
            ...
            params: { course: 'Child Minding', selected: true }
        }); 
        }}
      >
